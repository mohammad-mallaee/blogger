...
const DefaultCard = ({ post }: { post: PostData }) => {
  return (
    <Link className='group flex px-1 py-4 rounded-[4px] flex-col hover:scale-[1.01]
...
const CompactCard = ({ post }: { post: PostData }) => {
  return post.image ? (
    <Link className='group flex px-1 py-4 rounded-[4px] hover:scale-[1.01] gap-3 flex-col sm:flex-row
          ease-in-out transition-transform bg-[--surface] text-[--on-surface] border-[--outline]'
      style={{ direction: getMdDirection(post) }}
      key={post.slug} href={post.slug}>
...
const MinimalCard = ({ post }: { post: PostData }) => {
  return <Link className='group flex px-1 py-4 rounded-[4px] flex-col hover:scale-[1.01]
  ease-in-out transition-transform bg-[--surface] text-[--on-surface] border-[--outline]'
    style={{ direction: getMdDirection(post) }}
    key={post.slug} href={post.slug}>
