...
export default function Sidebar({ data }: { data: SidebarData }) {
    ...
    return <>
        ...
        <div className={`fixed top-0 lef-0 transition-opacity duration-300 ${context.isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            ...
            <div className={`fixed top-0 left-0 z-10 h-screen max-h-screen overflow-scroll no-scrollbar w-full max-w-[320px] xl:hidden bg-background px-4 py-8 transform transition-transform duration-300 ${context.isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <Main data={data} />
            </div>
        </div>
